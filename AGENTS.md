...
- Keep the flat worker browser tool surface provider-neutral. Isolate provider-specific implementations under `agent/subagents/worker/browser-providers`, select them only through `src/lib/browser-provider.ts`, and share lazy SDK clients through `src/lib/kernel.ts` and `src/lib/browserbase.ts`. Do not add a provider extension or root browser connection.
...
- Validate runtime environment variables through `src/env.ts`. `BROWSER_PROVIDER` defaults to `kernel`; require only the selected provider's API key. `BROWSERBASE_PROJECT_ID` is optional when its API key resolves to one project.
